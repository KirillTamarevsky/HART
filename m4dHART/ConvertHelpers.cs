...
namespace m4dHART
{
    ...
    {
        public static byte[] Single_to_HART_bytearray(this float number)
        {
            var bytes = BitConverter.GetBytes(number);
            var data = bytes.Reverse().ToArray();
            return data;
        }
        public static float To_HART_Single(this byte[] data)
        {
        ...
        public static string HART_unpack_string(this byte[] bytes)
        {
            ...
            for (int i = 0; i < chunks * 3; i += 3)
            {
                ...
                var chunk = Encoding.ASCII.GetString(new byte[] { (byte)c1, (byte)c2, (byte)c3, (byte)c4 });
